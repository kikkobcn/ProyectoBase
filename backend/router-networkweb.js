...
function createAccount(DIR_NODE) {
  ...
  execSync(
    `geth  --datadir ${DIR_NODE}  account new --password ${DIR_NODE}/pwd`
  );
...
router.get("/procesos/:network", async (req, res) => {
  ...
  return {
    numeroRed: paramsNodo.nodo.network,
    chainId: paramsNodo.nodo.chainId,
    numeroNodo: paramsNodo.nodo.nodo,
    puerto: paramsNodo.nodo.http_port,
  };
...
router.get("/:network", async (req, res) => {
  ...
  console.log(nodos);
  const output = nodos.map((i) => {
    const paramsNodo = JSON.parse(
      fs.readFileSync(`${NETWORK_DIR}/${i.name}/paramsNodo.json`)
    );
    return {
      numeroRed: paramsNodo.nodo.network.slice(3, red.length),
      chainId: paramsNodo.nodo.chainId,
      numeroNodo: paramsNodo.nodo.nodo,
      puerto: paramsNodo.nodo.http_port,
    };
